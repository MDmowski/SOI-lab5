 ...
 */
  ...
  if(!WorstFitFlag)
    ...
    register struct hole *maxHole, *maxHole_prev_ptr = NIL_HOLE;
    hp = maxHole = hole_head;
    while( hp != NIL_HOLE && hp->h_base < swap_base ){
      if( hp->h_len > maxHole->h_len ){
        maxHole = hp;
        maxHole_prev_ptr = prev_ptr;
      }
      prev_ptr = hp;
      hp = hp->h_next;
    }
    if( maxHole->h_len >= clicks ){ /* Enough room - using it */
      old_base = maxHole->h_base;
      maxHole->h_base += clicks;
      maxHole->h_len  -= clicks;

      if ( maxHole->h_len == 0 ) del_slot(maxHole_prev_ptr, maxHole); /* Delete if used all the space */
